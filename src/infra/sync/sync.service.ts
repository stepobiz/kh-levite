...
import { NodeRepository } from 'src/auto-engine/node/node.repository';
import { AuenNodeWithAttributes } from 'src/auto-engine/logic-engine/node-strategy.interface';
import { StrategyFactory } from 'src/auto-engine/logic-engine/strategy.factory';
...
export class SyncService implements OnModuleInit {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => NodeRepository)) private readonly nodeRepository: NodeRepository,
  ...
  async process(): Promise<number> {
    const allNodes = await this.nodeRepository.findAllWithAttributes() as unknown as AuenNodeWithAttributes[];
      ...
      try {
        ...
        if (direction === 'WRITE') {
          ...
          if (lastLog && lastLog.value !== node.actualValue) {
            await this.nodeRepository.setDesiredFromHardware(node.id, lastLog.value);
            count++;
          }
        }
      } catch (err) {
